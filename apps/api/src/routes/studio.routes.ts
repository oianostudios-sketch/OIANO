...
studioRouter.get('/current', authenticate, async (req: any, res, next) => {
  try {
    ...
    if (req.userRole === 'STUDIO_ADMIN' || req.userRole === 'ENGINEER') {
      ...
      // An artist with no bookings belongs to no studio. OIANO issues the
      // identity, so a newcomer is never placed at a default studio; the pages
      // that read this fall back to OIANO's own name when it is null.
