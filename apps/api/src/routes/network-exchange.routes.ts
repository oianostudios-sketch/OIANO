...
networkExchangeRouter.get('/',async(req:any,res:Response,next:NextFunction)=>{
  try{
    ...
    if(role==='STUDIO_ADMIN'){
      ...
      // status filter matches network-metrics.routes.ts's artists_served —
      // a cancelled booking isn't real activity, network-wide or per-studio.
      const recentBookings=await prisma.booking.findMany({where:{studio_id:studio.id,created_at:{gte:since},status:{notIn:['CANCELLED','NO_SHOW']}},select:{artist_id:true,starts_at:true,service:{select:{name:true}},artist:{select:{passport:{select:{creative_dna:true}}}}}});
    ...
    if(role==='ARTIST'){
      const studios=await prisma.studio.findMany({include:{rooms:true,services:true,bookings:{where:{created_at:{gte:new Date(Date.now()-30*86400000)},status:{notIn:['CANCELLED','NO_SHOW']}},select:{id:true,artist_id:true}}}});
