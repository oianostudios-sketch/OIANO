...
export default function StudioPassportPage() {
  ...
  if (isLoading) return <main className="min-h-screen grid place-items-center bg-[#08090a] text-zinc-600">Verifying studio…</main>;
  if (isError || !studio) return <main className="min-h-screen grid place-items-center bg-[#08090a] text-zinc-500">Studio Passport unavailable.</main>;
    ...
    <main className="min-h-screen bg-[#08090a] text-white">
      ...
      <div className="mx-auto max-w-6xl space-y-10 px-6 py-10">
        <section className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/[.07] bg-white/[.07] md:grid-cols-4">
          ...
          ].map(([Icon, value, label]: any) => <div key={label} className="bg-[#101112] p-6"><Icon size={15} className="mb-5 text-[#C9A84C]"/><strong className="block text-2xl">{value}</strong><span className="mt-1 block text-[10px] text-zinc-600">{label}</span></div>)}
        </section>

        <section><p className="mb-4 text-[10px] font-mono uppercase tracking-[.2em] text-zinc-600">Rooms & capability</p><div className="grid gap-4 md:grid-cols-2">{studio.rooms.map((room: any) => <article key={room.id} className="overflow-hidden rounded-2xl border border-white/[.07] bg-[#101112]">{room.image_url && <img src={room.image_url} alt={room.name} className="h-48 w-full object-cover"/>}<div className="p-5"><h2 className="text-lg">{room.name}</h2><p className="mt-2 text-xs leading-5 text-zinc-500">{room.description ?? 'Professionally managed production space.'}</p><div className="mt-4 flex flex-wrap gap-2">{room.amenities?.map((item: string) => <span key={item} className="rounded-full border border-white/[.07] px-3 py-1 text-[9px] text-zinc-500">{item}</span>)}</div></div></article>)}</div></section>

        <section className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]"><div><p className="mb-4 text-[10px] font-mono uppercase tracking-[.2em] text-zinc-600">Services</p><div className="space-y-2">{studio.services.map((service: any) => <div key={service.id} className="flex items-center justify-between rounded-xl border border-white/[.06] bg-[#101112] p-4"><div><strong className="text-sm font-medium">{service.name}</strong><p className="mt-1 text-[10px] text-zinc-600">{service.description ?? service.category.replaceAll('_',' ')}</p></div><span className="text-xs text-[#C9A84C]">from ${Number(service.min_price_usd).toFixed(0)} / {service.unit}</span></div>)}</div></div><div><p className="mb-4 text-[10px] font-mono uppercase tracking-[.2em] text-zinc-600">Studio Circle</p><div className="rounded-2xl border border-white/[.06] bg-[#101112] p-5">{studio.circle.length ? <div className="grid grid-cols-3 gap-4">{studio.circle.map((member: any, index: number) => <div key={`${member.artist.name}-${index}`} className="text-center"><ArtistAvatar name={member.artist.name} src={member.artist.avatar_url} size={48}/><p className="mt-2 truncate text-[10px] text-zinc-400">{member.artist.name}</p>{member.session_count && <p className="text-[8px] text-zinc-700">{member.session_count} sessions</p>}</div>)}</div> : <p className="text-xs leading-5 text-zinc-600">Artists appear here only after explicitly joining this studio’s Circle.</p>}</div></div></section>
