...
function Metric({ label, value, detail, icon: Icon, tone = 'blue', onClick }: { label: string; value: string | number; detail: string; icon: typeof Users; tone?: 'blue'|'gold'|'amber'; onClick:()=>void }) {
  const color = tone === 'gold' ? '#C9A84C' : tone === 'amber' ? '#f59e0b' : '#5A9BCB';
  return <button onClick={onClick} className="metric-enter group rounded-2xl border border-white/[.065] bg-studio-surface p-5 text-left transition hover:-translate-y-0.5 hover:border-white/[.12] focus:outline-none focus:ring-2 focus:ring-dome/20">
    <div className="flex items-center justify-between"><span className="grid h-8 w-8 place-items-center rounded-lg bg-white/[.035]"><Icon size={15} style={{color}} /></span><ChevronRight size={13} className="text-zinc-800 transition group-hover:text-zinc-500"/></div>
    <p className="mt-7 text-3xl font-semibold tracking-tight">{value}</p><p className="mt-2 text-[10px] font-mono uppercase tracking-[.16em] text-zinc-500">{label}</p><p className="mt-2 text-xs text-zinc-700">{detail}</p>
  </button>;
}
